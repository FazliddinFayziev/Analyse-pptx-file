...
def create_column_chart(products, data, colors, chart_title='Product Distribution', 
                        output_filename='ColumnChart.pptx', width=4, height=3, show_values=True, position='center'):
    ...
    # Add Column chart to the slide
    chart_data = CategoryChartData()
    chart_data.categories = products
    chart_data.add_series('Series 1', data)

    chart = slide.shapes.add_chart(
        chart_type=XL_CHART_TYPE.COLUMN_CLUSTERED, x=x, y=y, cx=cx, cy=cy, chart_data=chart_data
    ).chart
    ...
    # Show values on top of the columns
    if show_values:
        for i, point in enumerate(chart.series[0].points):
    ...
    print(f"Column chart created and saved in '{output_filename}'")
...
create_column_chart(products, data, colors, width=8, height=6, show_values=True, position='center')
